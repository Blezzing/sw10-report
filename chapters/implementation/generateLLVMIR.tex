...
To be able to run any code, we must first generate it. We generate \textit{LLVM Intermediate Representation}, which we now refer to as \textit{IR}, which can later be transformed to \textit{PTX} code, and be executed. We use the tools provided by the \textit{LLVM} library to generate this \textit{IR}, as they manage details such as variable names and references.

The \textit{IR} is based on a nested structure as seen on \ref{fig:llvmTypes}, where \texttt{module} is the program, \texttt{function} is a function that is a member of the module,  \texttt{basic block} is a code block, and \texttt{instruction} is a instruction. 
...
When a \texttt{Vector}s \texttt{exec()} function is called, the actions that have been created is used as source for the \textit{IR} generation. The procedure for \textit{IR} generation is as follows:
...
The implementation of the procedure is presented in this section.
...
Before we can create any \textit{LLVM} objects, we need a \texttt{LLVMContext}. A \texttt{LLVMContext} is a container of the state that \textit{LLVM} is in, and is declared at line \ref{code:irModule:context}.

With the \texttt{context} we create the module in the constructor chain at line \ref{code:irModule:module}. Next the module need to define a target architecture. We provide the string \texttt{"nvptx64-nvidia-cuda"}, as it is a key in the \textit{LLVM} library, that will be used to look up target information when code is generated. 
...
With the module created, we can start building \textit{LLVM Function}s to create our kernels.
...
To make sure the function follows the calling convention of the platform we can use the \texttt{setCallingConvention} function, as seen on line \ref{code:createKernelInitial:cc}. This means that parameter parsing is done according to convention for us.
...
With the function created we can start building \textit{LLVM Basic Block}s in them, to provide functionality to the kernels.

\subsection{Generating a LLVM Basic Block Based On an Action} \label{sec:bbbuilding}
The functionality needed of a kernel is dictated by the actions described in section \ref{cha:queueingActions}. All actions have a function that, when provided the module container, a pointer to the \textit{LLVM Function}, and an index value to indicate which of the kernel parameters, if any, is needed. This function can be seen on line \ref{code:addAction:func}.

First we create a \textit{LLVM Basic Bloc} at line \ref{code:addAction:bbc}, by providing the \texttt{LLVM Context} of the \textit{IR} container, a name, and the function that should contain it. We then create a \texttt{LLVM IRBuilder}, which is a object that provides functions for generating \textit{IR} from \textit{LLVM} objects, and add the block to the vector of blocks, that should be managed by the \textit{IR} container.

After constructing the \textit{LLVM Basic Bloc}, we get a pointer to the first kernel parameter, which always is the vector we are manipulating. We can now start building the \textit{IR}.

We use the \texttt{IRBuilder} to create the following instructions:
\begin{itemize}
...
\item Line \ref{code:addAction:const} creates a constant value based on the value the AddAction was created with.
\item Line \ref{code:addAction:add} creates the \texttt{add} instruction for floating point values, that adds the constant to the loaded value, and assigns a temporary value the result.
...
The \textit{LLVM Function} is ready to be executed on the GPU, but is not marked as a GPU kernel yet. To do this we use \textit{LLVM Metadata}, as the \textit{NVPTX} back end only requires us to provide the function we want marked as a kernel. On listing \ref{code:updateMetadata} we show how we use the \textit{LLVM API} to create a metadata node tagging our function, named "kernel", as a kernel function.
...
At line \ref{code:exec:construct} we construct the \texttt{IRModule}, which will construct a \textit{LLVM Module} ready for \textit{LLVM Function} insertion, as described in section \ref{sec:irmodule}.
...
At line \ref{code:exec:translate} we can translate the constructed \textit{IR} to \textit{PTX}, and be ready to execute it. How the translation is performed is covered in section \ref{dead}.

\todo{make this translation sections}
