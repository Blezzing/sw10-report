...
\textit{PACXX} uses \textit{LLVM} to generate, \textit{PTX} an \textit{SPIR} code at run-time. This is interesting since provides more opportunities and freedom for abstractions than a static header library would. \textit{PACXX} is also multi staged such that run-time compiled kernels can vary depending on state.
