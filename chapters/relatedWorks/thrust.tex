...
\textit{Thrust} is a header only implementation, but depends on the \textit{CUDA Run-time}, and as such requires the \textit{NVIDIA} compiler \textit{nvcc} to be used, forcing the developers choice of compiler.